...
def ssncut_full_search(A, k, ml_mat, cl_mat, a_list, b_list):
    best_a = 0
    best_b = 0
...
if __name__ == '__main__':
    ...
    alpha_list = [0.0, 1.0, 5.0, 10.0, 20.0, 30.0, 40.0] if not args.a else [args.a]
    ...
    if not args.priori_ml_list and not args.priori_cl_list:
        if args.use_sfs:
            ...
            a,b,res=ssncut_full_search(affinity_mat,bestK, sfs_mat,[] , alpha_list, [])
    ...
    if not args.priori_ml_list and args.priori_cl_list:
        ...
        if args.use_sfs:
            ...
            a,b,res=ssncut_full_search(affinity_mat, bestK, sfs_cl, [], alpha_list, [])
            logger.info("alpha:\t"+str(a))
            save_result(res,args.output,namelist)
